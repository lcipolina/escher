...
def select_solution(Z, limit=1):
    ...
    print('entering select_solution')

    # Set pixels out of bounds to (0,0)
    indx = (np.abs(Z.real) > limit * 0.98) | (np.abs(Z.imag) > limit * 0.98)
    Z[indx] = 0
...
def map_input(Z, src, src_scale, droste_scale):
    ...
    for ii in reversed(range(len(src))):
        x0 = ((x * src_scale[ii] + 1) * (src[ii].shape[1] / 2)).astype(np.float32)
        y0 = ((y * src_scale[ii] + 1) * (src[ii].shape[0] / 2)).astype(np.float32)
...
if __name__ == "__main__":
   ...
   resolution = 40            # larger number means more output pixels
   zoom = 1.7       # this value can be used to zoom in our out of the generated image. Should be greater than zero

   # Image data to map
   src = []
   src_scale = []

   # Compose all files in given folder
   # Note: images should be of the same size, so they will use the same soace on output. Do not resize them, crop them.
   path = '/Users/lucia/Desktop/Escher_Project/00-CODIGOS/0000-Cris_Luengo/00-Lucia/Test/completed'

   ii = 0
   lst = os.listdir(path) #files need to be read in order
   lst.sort()
   for filename in lst:
      # for ii in range(8):
