...
def map_input(Z, src, src_scale, droste_scale):
    ...
    unused = Z(0).Similar("BIN")
    unused.Fill(1)
    out = Z(0).Similar('SFLOAT')
    out.Fill(0)
    for ii in reversed(range(len(src))):
        Z0 = dip.MultiplySampleWise(Z + (1 / src_scale[ii]), [src_size[0] * src_scale[ii] / 2, src_size[1] * src_scale[ii] / 2])
        ...
        if dip.Any(mask)[0][0]:
           out[mask] = tmp[mask]
           unused ^= mask
    print("Fracation of pixels not assigned:", dip.Count(unused) / unused.NumberOfPixels())
    out.Convert("UINT8")
    return out
