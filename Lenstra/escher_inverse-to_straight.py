...
def map_input(Z, src, limit=1):
    ...
    x = (x / limit + 1) * src.shape[1] / 2
    y = (y / limit + 1) * src.shape[0] / 2
...
if __name__ == "__main__":
   ...
   resolution = 1    # larger number means more output pixels - it takes longer but increases the resol and img size
   ...
   for zoom in range(2):  #'zoom' parameter controls how much inward in the center we want to go - we need at least 6 outputs to get inside the painting (into the gallery)
      # Generate mapped image
      rot_angle = 0.65 #0 #0.65  #experimental (to make output straight)
      out = escher_inverse(src, 2**(-zoom), resolution, rot_angle) # rotation angle 0.65 determined experimentally
      # Write to file
      print('writing to file - '+str(zoom))
      cv2.imwrite('escher_straightened_'+str(zoom)+'.png', out)
